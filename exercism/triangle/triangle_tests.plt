...
:- begin_tests(equilateral_triangle).
    ...
    test(any_side_is_unequal, condition(false)) :-
        triangle(2, 3, 2, "equilateral").

    test(no_sides_are_equal, condition(false)) :-
        triangle(5, 4, 6, "equilateral").

    test(all_zero_sides_are_not_a_triangle, condition(false)) :-
...
:- begin_tests(isosceles_triangle).
    ...
    test(first_triangle_inequality_violation, condition(false)) :-
        triangle(1, 1, 3, "isosceles").

    test(second_triangle_inequality_violation, condition(false)) :-
        triangle(1, 3, 1, "isosceles").

    test(third_triangle_inequality_violation, condition(false)) :-
...
:- begin_tests(scalene_triangle).
    ...
    test(all_sides_are_equal, condition(false)) :-
        triangle(4, 4, 4, "scalene").

    test(two_sides_are_equal, condition(false)) :-
        triangle(4, 4, 3, "scalene").

    test(may_not_violate_triangle_inequality, condition(false)) :-
